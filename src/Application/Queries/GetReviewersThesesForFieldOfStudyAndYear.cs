...
public static class GetReviewersThesesForFieldOfStudyAndYear
{
    ...
    {
        private const string SqlQuery = "select the.thesis_id as \"Id\", t.\"name\" as \"Name\", t.english_name as \"EnglishName\", supervisor .academic_degree || ' ' || supervisor.first_name || ' ' || supervisor.last_name as \"SupervisorName\", realizer.first_name || ' ' || realizer.last_name || ' ' || realizer.index_number as \"StudentName\", the.status as \"Status\", supervisor.department as \"Department\" from topic as t join thesis as the on t.topic_id = the.topic_id join review as rev on rev.thesis_id = the.thesis_id join \"user\" as reviewer on reviewer.user_id = rev.reviewer_id join \"user\" as realizer on realizer.user_id = the.realizer_student_id join \"user\" as supervisor on t.supervisor_id = supervisor.user_id where t.year_of_defence = :YearOfDefence and t.field_of_study_id = :FieldOfStudyId and reviewer.user_id = :TutorId ORDER BY realizer.last_name asc, realizer.first_name asc OFFSET :OffsetRows ROWS FETCH NEXT :ItemsPerPage ROWS ONLY";
        ...
        public async Task<PagedResultDto<ReviewersThesisDto>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            var results =
                await connection
                    .QueryAsync<ReviewersThesisDto>(SqlQuery, new
                    {
